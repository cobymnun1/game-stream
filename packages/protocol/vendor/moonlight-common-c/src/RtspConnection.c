...
static bool transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, int* error) {
    ...
    *error = -1;
    ...
    /* #region agent log */
    ml_dbg("tcp_after_connect", (int)sock, ConnectionInterrupted ? 1 : 0);
    /* #endregion */
    if (sock == INVALID_SOCKET) {
        return ret;
    }
    ...
    /* #region agent log */
    ml_dbg("tcp_after_seal", serializedMessage != NULL ? 1 : 0, messageLen);
    /* #endregion */
    if (serializedMessage == NULL) {
        closeSocket(sock);
        sock = INVALID_SOCKET;
    ...
    /* #region agent log */
    ml_dbg("tcp_after_send", (int)err, messageLen);
    /* #endregion */
    if (err == SOCKET_ERROR) {
        *error = LastSocketError();
...
int performRtspHandshake(PSERVER_INFORMATION serverInfo) {
    ...
    /* #region agent log */
    ml_dbg("handshake_setup", useEnet ? 1 : 0, (int)RtspPortNumber);
    /* #endregion */
