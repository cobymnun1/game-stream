...
/* #region agent log */
extern void ml_dbg(const char* tag, int a, int b);
/* #endregion */
...
static void VideoPingThreadProc(void* context) {
    ...
    int pingCount = 0;
    /* #region agent log */
    ml_dbg("video_ping_thread_start", (int)rtpSocket, (int)VideoPortNumber);
    /* #endregion */
    while (!PltIsThreadInterrupted(&udpPingThread)) {
        ...
        if (VideoPingPayload.payload[0] != 0) {
            ...
            pingRet = sendto(rtpSocket, (char*)&VideoPingPayload, sizeof(VideoPingPayload), 0, (struct sockaddr*)&saddr, AddrLen);
        }
        else {
            pingRet = sendto(rtpSocket, legacyPingData, sizeof(legacyPingData), 0, (struct sockaddr*)&saddr, AddrLen);
        }
        /* #region agent log */
        if (pingCount <= 1) ml_dbg("video_ping_sendto_ret", pingRet, (int)rtpSocket);
        /* #endregion */
...
static void VideoReceiveThreadProc(void* context) {
    ...
    waitingForVideoMs = 0;
    /* #region agent log */
    ml_dbg("video_recv_thread_start", (int)rtpSocket, useSelect ? 1 : 0);
    /* #endregion */
